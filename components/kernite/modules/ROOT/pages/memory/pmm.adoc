...
| `UntypedReserved` | Frame is carved out of an `UntypedMemory` watermark but has not been promoted to a typed owner yet (reset / retype windows, device untypeds).  Blocks double allocation while `untyped` still claims the backing range.
