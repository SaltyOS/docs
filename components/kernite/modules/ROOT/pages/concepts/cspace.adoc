...
On aarch64, I/O port slots are reused for MMIO-oriented objects (PL011 device untyped, interrupt handler).

Child processes spawned by `procmgr` receive a different well-known slot layout:
...
| 8 | `CAP_COM1_IOPORT` | Serial port I/O
