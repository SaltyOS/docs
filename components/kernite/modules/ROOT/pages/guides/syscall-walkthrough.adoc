...
. *Switch stacks*: load the per-thread kernel stack pointer from the TCB.  The user stack pointer is saved on the kernel stack (not in a per-CPU global, to prevent corruption during context switches).
