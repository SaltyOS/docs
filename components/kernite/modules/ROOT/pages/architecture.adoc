...
| `syscall/`
| System call dispatch: 28 syscalls, capability invocation, IPC fastpath
...
| `fpu.rs` | x87/SSE/AVX lazy context switch via `#NM` trap
| `pit.rs` | Legacy PIT for APIC timer calibration
| `uaccess.rs` | SMAP/SMEP user memory access control
|===

Syscall entry: `syscall` instruction.  Entry point in `syscall.S` saves user RSP on the per-thread kernel stack (not per-CPU `%gs:16`) to prevent corruption during context switches.
...
| `ap_boot.rs` | AP startup via PSCI `CPU_ON` (HVC) + mailbox handoff
...
| `fpu.rs` | NEON Q0-Q31 (512 bytes) lazy switch via `CPACR_EL1` trapping
...
FPU/SIMD state is managed via lazy context switching.
On x86_64, the kernel traps `#NM` (Device Not Available) exceptions.
On aarch64, CPACR_EL1 trapping detects the first FPU use after a context switch.
